...
void forward(ann_t *nn, double (*activation_function)(double))
{
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        cudaMalloc((void **)&z1,  nn->layers[l]->number_of_neurons * nn->minibatch_size * sizeof(double));
        cudaMalloc((void **)&z2,  nn->layers[l]->number_of_neurons * nn->minibatch_size * sizeof(double));
        cudaMalloc((void **)&one, nn->minibatch_size * sizeof(double));

        cudaMemset(one, (double) 1, nn->minibatch_size * sizeof(double));

        double *weights; 
        double *biases;
        double *activations;

        cudaMalloc((void **)&weights,  nn->layers[l]->weights->rows * nn->layers[l]->weights->columns * sizeof(double));
        cudaMalloc((void **)&biases,  nn->layers[l]->biases->rows * nn->layers[l]->biases->columns * sizeof(double));
        cudaMalloc((void **)&activations,  nn->layers[l]->activations->rows * nn->layers[l]->activations->columns * sizeof(double));

        cudaMemcpy(weights, nn->layers[l]->weights->m, nn->layers[l]->weights->rows * nn->layers[l]->weights->columns * sizeof(double), cudaMemcpyHostToDevice);
        cudaMemcpy(weights, nn->layers[l]->biases->m, nn->layers[l]->biases->rows * nn->layers[l]->biases->columns * sizeof(double), cudaMemcpyHostToDevice);
        cudaMemcpy(weights, nn->layers[l]->activations->m, nn->layers[l]->activations->rows * nn->layers[l]->activations->columns * sizeof(double), cudaMemcpyHostToDevice);

        matrix_dot_shared_memory(weights, activations, z1); // z1 <- w^l x a^(l-1)
        matrix_dot_shared_memory(biases, one, z2); // z2 <- b^l x 1        
